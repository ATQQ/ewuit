...
class ElementTool {
  ...
  refreshModal() {
    ...
    // 数据格式化
    data = attrs.map((v) => stylePropConvert(v, styles[v] as string, {
    ...
    ${data
    .map((v) => `<p><span>${v.label}:</span> ${v.value}</p>`)
    .join('')}
    </div>`;
    ...
    setTimeout(() => {
      ...
      // 展示位置计算
      const left = domRect.left < screenWidth / 2 ? unitValue(domRect.left + bufferWidth) : 'auto';
      const top = domRect.bottom + safeTopDistance < screenHeight
        ? unitValue(domRect.bottom + verticalDistance + bufferHeight)
        : 'auto';
      const right = left === 'auto' ? unitValue(screenWidth - domRect.left - domRect.width + bufferWidth) : 'auto';
      const bottom = top === 'auto'
        ? unitValue(screenHeight - domRect.top + verticalDistance + bufferHeight)
        : 'auto';
