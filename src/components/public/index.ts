...
import { Toast } from '..';
import {
  getDomRects, getScreenWidth, updateDomStyles, unitValue,
} from '@/utils';
...
export function updateCoverDom(
  ...
  // 高度展示
  coverDom.style.setProperty('--ewuit-comp-cover-left', isSafeWidth ? 'auto' : '-46px');
  coverDom.style.setProperty('--ewuit-comp-cover-right', isSafeWidth ? '0' : 'auto');

  const isSafeHeight = top > 20;
  // 宽度展示
  coverDom.style.setProperty('--ewuit-comp-cover-bottom', isSafeHeight ? 'auto' : '20px');
  coverDom.style.setProperty('--ewuit-comp-cover-top', isSafeHeight ? '-20px' : 'auto');
}
