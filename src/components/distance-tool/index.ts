...
class DistanceTool {
  ...
  handle(e: MouseEvent) {
    ...
    // 是否连续点击两个一样的
    const isSameClick = domStack.slice(-1)[0] === clickDom
    ...
    setRootStyleProperty(
      '--cover-size-display',
      domStack.length === 1 ? 'block' : 'none'
    )
    // 控制距离展示
    setRootStyleProperty(
      '--cover-distance-display',
      domStack.length === 1 ? 'none' : 'block'
    )
  ...
  refreshRulers() {
    ...
    const { left, width, height, top } = getDomRects(targetDom)
  ...
  refreshCover() {
    ...
    setRootStyleProperty(
      ...
      getComputedStyle(first).borderRadius
    )
  ...
  refreshDistance() {
    ...
    if (
      cData.top < tData.bottom &&
      cData.top > tData.top &&
      cData.bottom < tData.bottom
    ) {
    ...
    if (
      cData.left < tData.right &&
      cData.left > tData.left &&
      cData.right <= tData.right
    ) {
