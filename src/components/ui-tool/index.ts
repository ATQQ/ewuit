...
import { addStyleDom, addToHtml, h, preventDomDefault } from '@/utils'
...
export default class EwuitUITool {
  ...
  static bindTogglePanel(e: MouseEvent): void {
    // 获取旧状态
    const oldStatus = this.unIndependenceButtonList.reduce((pre, current) => {
      pre[this.btnTypeMap.get(current) as string] =
    ...
    // 后执行打开
    // 执行自定义回掉
    Object.keys(diffStatus)
      .sort((k1, k2) => diffStatus[k1] - diffStatus[k2])
      .forEach((k) => {
